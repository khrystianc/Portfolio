...
tree_print_recurse(struct fileinfo finfo)
{
  ...
  /* product of the current "depth" option */
  for (int i = 0; i < 2 * depth; ++i) putchar(' ');

  /* TODO: print the path info */
  /*Print_pathinfo function below */
  /* permissions, user, groups, size and path name */
  /* permssions, check loadstring function below */
  /* if it is a file, you are done and return, continue below if it is a directory */

  
  /* TODO: continue ONLY if path is a directory */
  /* recurse into directory: open, change cur_dir vglobal variable to point at the directory nd read all files in the directory */
  /* read_file list is opened, so it needs to be closed */
